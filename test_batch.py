...
openBatch1 = OpenBatch(5, [commandPlan1, commandEdit1])


def test_OpenBatch_init():
    with pytest.raises(AssertionError):
        no_commands = OpenBatch(openBatch1.id, [])
    with pytest.raises(AssertionError):
        no_plans = OpenBatch(openBatch1.id, [commandEdit1])
    no_finishes = OpenBatch(openBatch1.id, [commandPlan1])

def test_OpenBatch_eq_same():
    assert openBatch1 == openBatch1

def test_OpenBatch_eq_equal():
    assert openBatch1 == OpenBatch(5, [commandPlan1, commandEdit1])
...
def test_OpenBatch_eq_different_id():
    assert openBatch1 != OpenBatch(6, openBatch1.command_records)

def test_OpenBatch_eq_different_command_records():
    assert openBatch1 != OpenBatch(openBatch1.id, [commandPlan1])
